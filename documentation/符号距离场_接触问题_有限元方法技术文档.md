# NexDynFEM 技术文档：符号距离场、接触问题与有限元方法
...
#### 1.1.3 在NexDynFEM中的应用场景

在NexDynFEM项目中，符号距离场主要应用于：
...
核心实现类包括：
- [DistanceField](file:///e:/workspace/NexDynFEM/libraries/distanceField/distanceField.h)：基础距离场类
- [DistanceFieldBase](file:///e:/workspace/NexDynFEM/libraries/distanceField/distanceFieldBase.h)：距离场基类
- [SignedDistanceFieldFromPolygonSoup](file:///e:/workspace/NexDynFEM/libraries/distanceField/signedDistanceFieldFromPolygonSoup.h)：从多边形汤计算符号距离场
- [DistanceFieldCreator](file:///e:/workspace/NexDynFEM/libraries/distanceField/distanceFieldCreator.h)：距离场创建器
...
#### 1.2.4 在NexDynFEM中的实现

NexDynFEM通过以下方式处理接触问题：
...
2. **连续碰撞检测（CCD）**：
   - [pointCCD](file:///e:/workspace/NexDynFEM/libraries/distanceField/distanceField.h#L212) 方法计算最早接触时间
   - 防止高速运动时的穿透

3. **网格相交检测**：
   - [meshIntersection](file:///e:/workspace/NexDynFEM/libraries/mesh/meshIntersection.h) 处理网格相交
   - [iglRemeshSelfIntersection](file:///e:/workspace/NexDynFEM/libraries/libiglInterface/iglRemeshSelfIntersection.h) 处理自相交
...
2. **共旋转线性有限元（Corotational Linear FEM）**：
   - [CorotationalLinearFEM](file:///e:/workspace/NexDynFEM/libraries/corotationalLinearFEM/corotationalLinearFEM.h) 类实现
   - 处理大变形问题
   - 支持三种模式：
     * warp=0：无旋转（线性FEM）
...
3. **各向同性超弹性有限元（Isotropic Hyperelastic FEM）**：
   - [IsotropicHyperelasticFEM](file:///e:/workspace/NexDynFEM/libraries/isotropicHyperelasticFEM/isotropicHyperelasticFEM.h) 类实现
   - 支持多种超弹性材料模型
   - 处理大变形和单元翻转
...
2. **正交各向异性材料**：
   - [Stable Orthotropic Materials](file:///e:/workspace/NexDynFEM/libraries/corotationalLinearFEM/corotationalLinearFEM.h#L51) [Li & Barbic 2014]
...
在NexDynFEM中，[WarpMatrix](file:///e:/workspace/NexDynFEM/libraries/corotationalLinearFEM/corotationalLinearFEM.h#L120) 函数实现刚度矩阵旋转：
...
在NexDynFEM中，[IsotropicHyperelasticFEM](file:///e:/workspace/NexDynFEM/libraries/isotropicHyperelasticFEM/isotropicHyperelasticFEM.h) 实现了基于块对角 dP/dF 张量的计算方法 [Teran 2005]。
...
2. **自相交处理**：
   - 使用距离场检测网格自相交
   - 使用 [iglRemeshSelfIntersection](file:///e:/workspace/NexDynFEM/libraries/libiglInterface/iglRemeshSelfIntersection.h) 修复网格
...
1. **距离场接口**：
   ...
   [DistanceField](file:///e:/workspace/NexDynFEM/libraries/distanceField/distanceField.h) 类提供以下关键接口：
...
2. **有限元接口**：
   ...
   [CorotationalLinearFEM](file:///e:/workspace/NexDynFEM/libraries/corotationalLinearFEM/corotationalLinearFEM.h) 类提供：
   ...
   [IsotropicHyperelasticFEM](file:///e:/workspace/NexDynFEM/libraries/isotropicHyperelasticFEM/isotropicHyperelasticFEM.h) 类提供：
...
3. **力模型接口**：
   ...
   [ForceModel](file:///e:/workspace/NexDynFEM/libraries/forceModel/forceModel.h) 基类定义了统一接口：
...
2. Jernej Barbic, Hongyi Xu, Yijing Li: "NexDynFEM Distance Field Library", USC, 2018
...
```cpp
#include <nexdynfem/distanceField.h>
...
**解决**：
1. 使用 [SignedDistanceFieldFromPolygonSoup](file:///e:/workspace/NexDynFEM/libraries/distanceField/signedDistanceFieldFromPolygonSoup.h) 类
2. 设置合适的 sigma 参数
3. 检查网格是否封闭
...
**解决**：
1. 使用 [IsotropicHyperelasticFEM](file:///e:/workspace/NexDynFEM/libraries/isotropicHyperelasticFEM/isotropicHyperelasticFEM.h) 并设置 inversionThreshold
2. 减小时间步长
3. 增加材料刚度
